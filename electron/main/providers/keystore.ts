...
export type ApiKeyProvider = 'openai' | 'anthropic' | 'hf' | 'openrouter' | 'together'

export async function getDecryptedApiKey(provider: ApiKeyProvider): Promise<string> {
  const db = getDb()
...
export async function getDecryptedApiKeyOrNull(provider: ApiKeyProvider): Promise<string | null> {
  try {
