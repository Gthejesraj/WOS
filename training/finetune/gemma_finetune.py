"""QLoRA fine-tuning — WOS Gemma 2-27B models.

Usage:
  python gemma_finetune.py --model coding
  python gemma_finetune.py --model meeting
  python gemma_finetune.py --model main
"""
...
def format_sample(example, tokenizer):
    ...
    if system_content and turns:
        turns[0]["content"] = f"{system_content}\n\n{turns[0]['content']}"
    return {"text": tokenizer.apply_chat_template(turns, tokenize=False, add_generation_prompt=False)}
...
def main():
    ...
    config_key = f"{args.model}-{BASE}"
    cfg = MODEL_CONFIGS[config_key]

    print(f"\n{'='*60}")
    print(f"Training WOS-{args.model.upper()} (Gemma 2-27B)")
    ...
    print("Loading model in 4-bit (~10 min for 27B)...")
    ...
    print(f"Saving full model to {merged_path}...")
    ...
    print(f"\n{'='*60}")
    print(f"WOS-{args.model.upper()} (GEMMA) COMPLETE!")
